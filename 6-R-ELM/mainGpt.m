...
N_runs = 1; % Cambiar a 5 para la validación final
...
datasets = {X_RELM_boruta, X_RELM_cbsfoa1, X_RELM_pi};
dataset_names = {'Boruta', 'CBSFOA1', 'PI'};
...
for d = 1:num_datasets
    ...
    for run = (ultima_run + 1) : N_runs
        ...
        for fold = 1:K
            ...
            % Normalización Z-score ajustada solo con datos de train
    ...
    % *** CAMBIO CLAVE: Buscar el máximo basado en MCC, no en Accuracy ***
    ...
    % Extraer los valores de Accuracy y G-Mean correspondientes al mejor MCC
    ...
    % Guardar en estructura para futuras tablas
    resultados_finales.(dataset_names{d}).mean_acc = mean_acc;
    resultados_finales.(dataset_names{d}).mean_mcc = mean_mcc;
    resultados_finales.(dataset_names{d}).mean_gmean = mean_gmean;
    resultados_finales.(dataset_names{d}).best_params = [best_Nh, best_C];
end

%% ================= FUNCIONES NÚCLEO RELM (PASO 2) ================= %%
