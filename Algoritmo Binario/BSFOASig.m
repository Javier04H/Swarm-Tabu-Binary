...
for i = 1:Npop
    ...
    TF = 1 ./ (1 + exp(-10 * (Xpos(i,:) - 0.5))); %Transfer Fuction - toma el valor y lo mapea en un rango de 0 y 1
    %Binarizamos
    binX = TF >=rand(1,nD);
...
bin_best = TF_best >= 0.5; % Usamos 0.5 estricto para el resultado final
Sf = find(bin_best == 1);  % Índices de las características
