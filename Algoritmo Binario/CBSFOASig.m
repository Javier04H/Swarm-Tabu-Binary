function [xposbest, fvalbest, Curve, Sf, Nf] = CBSFOASig(Npop, Max_it, lb, ub, nD, fobj)
...
for i = 1:Npop
    for j = 1:nD
        % Fórmula del Mapa Caótico Piecewise (PWLCM)
        if chaos >= 0 && chaos < p_map
            chaos = chaos / p_map;
        elseif chaos >= p_map && chaos < 0.5
            chaos = (chaos - p_map) / (0.5 - p_map);
        elseif chaos >= 0.5 && chaos < 1 - p_map
            chaos = (1 - p_map - chaos) / (0.5 - p_map);
        elseif chaos >= 1 - p_map && chaos < 1
            chaos = (1 - chaos) / p_map;
        end
